...
class Model():
    ...
    def __init__(self,vocab_size,num_classes,init_embedding_mat,max_doc_length
        ...
        with tf.name_scope("placeholder"):
            ...
            self.input_x_1 = tf.placeholder(tf.int32,[None,None],name="input_x_1")
            self.input_x_2 = tf.placeholder(tf.int32,[None,None],name="input_x_2")
        ...
        with tf.variable_scope(share_encoder_scope,reuse=tf.AUTO_REUSE):
            word_encoded_x2 = self.BidirectionalGRUEncoder(word_embedded_x2,"word_encoder_x2")
        ...
        weight_x2 = self.AttentionLayer(word_encoded_x2,word_context,W_word,b_word,name='attention_2')

#根据新的embedding,和 weights ，计算两个句子的距离。
        out = self.classifier(word_encoded_x1,weight_x1,word_encoded_x2,weight_x2)
    ...
    def classifier(self,word_encoded_x1,weight_x1,word_encoded_x2,weight_x2):
        ...
        expand_weight_x1 = tf.expand_dims(weight_x1,-1)
        expand_weight_x2 = tf.expand_dims(weight_x2,-1)
        atten_sen1 = tf.reduce_sum(tf.multiply(word_encoded_x1,expand_weight_x1),axis=1)
        atten_sen2 = tf.reduce_sum(tf.multiply(word_encoded_x2,expand_weight_x2),axis=1)
