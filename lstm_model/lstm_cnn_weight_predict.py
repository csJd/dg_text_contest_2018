...
# 预测文件路径
tf.flags.DEFINE_string("predict_filename","lstm_model/processed_data/one_gram/filter-1gram_phrase_level_data_200_dev.csv","predict_filename path")
        ...
        for step_i in [900,1000,1100,1200,1300,1400,1500,1600,1700,1800,1900,2000,2100,2200,2300]:
            ...
            with graph.as_default():
                ...
                with sess.as_default():
                    # 加载训练好的模型
                    # lstm_model / runs / 1535556239_2 / checkpoints / model - 900.
                    # meta
                    print("meta_path", "./runs/"+model_path[cv_num_i]+"/checkpoints/model-" + step + ".meta")
                    saver = tf.train.import_meta_graph("meta_path", "./runs/"+model_path[cv_num_i]+"/checkpoints/model-" + step + ".meta")
                    ...
                    # 写入文件
                    pk.dump(batch_prediction_pro_all, open(from_project_root("result_path", "lstm_model/result_test/result_predict" + cv_num + "-" + step + ".csv"), 'wb'))
